...
async function callAnthropic({ messages, config, temperature, stream, apiKey, reasoning }) {
  ...
  } else if (!['claude-opus-4-7', 'claude-opus-4-8', 'claude-opus-5', 'claude-opus-5-5', 'claude-fable-5', 'claude-fable-5-1', 'claude-mythos-5', 'claude-sonnet-5'].includes(config.modelName)) {
  ...
  if (thinkingBlock) {
    debugLog(`${config.displayName || config.modelName} THINKING: ${thinkingBlock.thinking?.substring(0, 500)}...`);
  }
  ...
  return { content: textBlock?.text || '', thinking: thinkingBlock?.thinking || null, usage };
}
...
async function callOpenAI({ messages, config, temperature, stream, apiKey, reasoning }) {
  ...
  if (data.choices?.[0]?.message?.reasoning_content) {
    debugLog(`${config.displayName || config.modelName} THINKING: ${data.choices[0].message.reasoning_content.substring(0, 500)}...`);
  }
  ...
  return {
    ...
    thinking: data.choices?.[0]?.message?.reasoning_content || null,
    usage
  };
}
