...
// Check if user is authorized to use default keys
export function isAuthorized(clientKeys = {}) {
  const isDevelopment = process.env.NODE_ENV === 'development';
  const hasAuthToken = process.env.AUTH_TOKEN &&
                       clientKeys.OPENAI_API_KEY === process.env.AUTH_TOKEN;
  return isDevelopment || hasAuthToken;
}
...
function getApiKey(provider, clientKeys = {}) {
  ...
  // If authorized, use server-side keys; otherwise use client keys
  if (isAuthorized(clientKeys)) {
    return process.env[envKey];
  }

  // For public users, require their own keys
  return clientKeys[envKey];
}
