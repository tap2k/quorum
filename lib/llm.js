...
export const modelConfigs = {
  ...
  'claude-haiku-3.5': {
    ...
    cost: { input: 0.80, output: 4.00 }, // per million tokens
  },
  'claude-haiku-3': {
    provider: 'anthropic',
...
export async function callLLM({
  ...
  try {
    ...
    switch (config.provider) {
      case 'anthropic':
        content = await callAnthropic({ messages: finalMessages, config, temperature, stream, apiKey, reasoning });
        break;
      case 'openai':
        content = await callOpenAI({ messages: finalMessages, config, temperature, stream, apiKey, reasoning });
        break;
      case 'google':
        content = await callGoogle({ messages: finalMessages, config, temperature, stream, apiKey, reasoning });
        break;
      case 'xai':
...
async function callAnthropic({ messages, config, temperature, stream, apiKey, reasoning }) {
  ...
  if (thinkingBlock) {
    console.log(`Claude THINKING: ${thinkingBlock.thinking?.substring(0, 500)}...`);
  }
...
async function callOpenAI({ messages, config, temperature, stream, apiKey, reasoning }) {
  ...
  if (isOSeries) {
    // O-series models (o1, o3, o4) use 'reasoning' parameter
  ...
  if (data.choices?.[0]?.message?.reasoning_content) {
    console.log(`OpenAI REASONING: ${data.choices[0].message.reasoning_content.substring(0, 500)}...`);
  }
...
async function callGoogle({ messages, config, temperature, stream, apiKey, reasoning }) {
  ...
  if (supportsThinking && reasoning && reasoning !== 'off') {
    ...
    if (isGemini3) {
      ...
      generationConfig.thinkingConfig = { thinkingLevel: levelMap[reasoning] || 'LOW' };
    } else {
