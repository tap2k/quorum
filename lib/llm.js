...
export async function callLLM({
  ...
  try {
    ...
    switch (config.provider) {
      case 'anthropic':
        content = await callAnthropic({ messages: finalMessages, config, temperature, stream, apiKey });
        break;
      case 'openai':
        content = await callOpenAI({ messages: finalMessages, config, temperature, stream, apiKey });
        break;
      case 'google':
        content = await callGoogle({ messages: finalMessages, config, temperature, stream, apiKey });
        break;
      case 'xai':
...
async function callOpenAI({ messages, config, temperature, stream, apiKey }) {
  ...
  // GPT-5 models and o-series models don't support custom temperature
  const useTemperature = !config.modelName.startsWith('gpt-5') && !config.modelName.startsWith('o');
