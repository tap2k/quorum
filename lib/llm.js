...
export async function callLLM({
  ...
  try {
    switch (config.provider) {
      case 'anthropic':
        return await callAnthropic({ messages: finalMessages, config, temperature, stream });
      case 'openai':
        return await callOpenAI({ messages: finalMessages, config, temperature, stream });
      case 'google':
        return await callGoogle({ messages: finalMessages, config, temperature, stream });
      case 'xai':
...
async function callAnthropic({ messages, config, temperature, stream }) {
...
async function callOpenAI({ messages, config, temperature, stream }) {
...
async function callGoogle({ messages, config, temperature, stream }) {
...
export async function callMultipleLLMs({
  ...
  const promises = models.map(model =>
    callLLM({ messages, model, temperature, systemPrompt })
